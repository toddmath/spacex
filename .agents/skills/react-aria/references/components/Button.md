...
The `onPressStart`, `onPressEnd`, and `onPressChange` events are also emitted as the user interacts with the button. Each of these handlers receives a `PressEvent`, which provides information about the target and interaction method. See [usePress](../interactions/usePress.md) for more details.
