...
Use the `name` and `value` props to submit the checkbox to the server. Set the `isRequired` prop to validate the user selects the checkbox, or implement custom client or server-side validation. See the [Forms](../guides/forms.md) guide to learn more.
...
| `inputRef` | `React.Ref<HTMLInputElement | null> | undefined` | — | A ref for the HTML input element. |
