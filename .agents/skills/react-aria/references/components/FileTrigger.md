...
`FileTrigger` works with any pressable React Aria component (e.g. [Button](./Button.md), [Link](./Link.md), etc.). Use the `<Pressable>` component or [usePress](../interactions/usePress.md) hook to wrap a custom trigger element such as a third party component or DOM element.
