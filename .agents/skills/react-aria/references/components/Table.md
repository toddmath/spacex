...
`Table` follows the [Collection Components API](../guides/collections.md?component=Table), accepting both static and dynamic collections.
...
Use the `href` prop on a `<Row>` to create a link. Link interactions vary depending on the selection behavior. See the [selection guide](../guides/selection.md) for more details.
...
| `className` | `string | undefined` | 'react-aria-TableLoadingIndicator' | The CSS [className](https://developer.mozilla.org/en-US/docs/Web/API/Element/className) for the element. |
| `dir` | `string | undefined` | — |  |
